...
class AttributeLibrary extends BaseLibrary {
  constructor() {
    super();
  ...
  fillLibrary(data, options = {}) {
    ...
    result.walk((root) => {
      ...
      if (root.selector && !parentName.match(/keyframes/)) {
        const matchedSelectors = root.selector.match(this.getAtttributeSelectorRegex()) || [];
  ...
  setAttributeSelector(attributeSelector) {
    ...
    const re = new RegExp(this.getAtttributeSelectorRegex());
  ...
  replaceAttributeSelector(selector) {
    ...
    return this.selectorFirstChar + result;

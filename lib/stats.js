...
const stats = () => {
  const classSelector = values(selectorsLibrary.getClassSelector());
  const idSelector = values(selectorsLibrary.getIdSelector());
  ...
  const unusedClasses = getUnused(classSelector.meta);
  const unusedIds = getUnused(idSelector.meta);
  ...
  Object.keys(classSelector.value).forEach((v) => {
    classUsageCount[v] = classSelector.meta[v].appearanceCount;
  });
  Object.keys(idSelector.value).forEach((v) => {

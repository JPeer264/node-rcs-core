...
const regex = {
    ...
    sizesWithDots: /\.([0-9]*?)(em|rem|%|vh|vw|s|cm|ex|in|mm|pc|pt|px|vmin)/g, // match everything which starts with . and has numbers in it and ends with em, rem,... - necessary for .9em or .10s
    ...
    strings: /('|")\s*[\s\S]*?\s*('|")/g // matches strings such as: 'hello' or "hello"
};
...
replace.fileCss = (filepath, options, cb) => {
    ...
    fs.readFile (filepath, (err, data) => {
        ...
        bufferObject = replace.bufferCss(data);

...
const regex = {
    ...
    keyframes: /@keyframes\s+([a-zA-Z0-9_-]+)/g, // matches keyframes and just the first group the first matched non-whitespace characters - e.g. matches: `@keyframes    my-KeyFra_me`
    animationTrigger: /(animation-name:|animation:)\s+([a-zA-Z0-9_-]+)/g, // matches `animation-name` or `animation` group them group[0] and also match the first word after group[1]
};
...
replace.bufferCss = (bufferObject, options) => {
    ...
    if (options.replaceKeyframes) {
        ...
        rcs.keyframesLibrary.set(
            data.match(regex.keyframes).map(key => key.split(/\s+/)[1]),
            options
        );
    }
    ...
    if (options.replaceKeyframes) {
        ...
        data = data.replace(regex.animationTrigger, replaceKeyframes);
    }

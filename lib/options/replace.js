...
replace.buffer = (bufferObject, options) => {
    ...
    data = data.replace(replace.regex.strings, match => {
...
/**
 * Parse a javascript buffer and returns the modified file
 *
 * @param  {String} bufferObject the buffer's path to change
 *
 * @return {Buffer}
 */
replace.bufferJs = (bufferObject, options) => {
    ...
    let data = bufferObject.toString();
    ...
    ast.tokens.forEach(token => {
        if (token.type === 'String' || (options.jsx && token.type === 'JSXText')) {
    ...
    data = data.replace(foundStringRegex, match => {
        return replace.string(match, regex);
    });
...
replace.bufferCss = (bufferObject, options) => {
    ...
    if (!options.ignoreAttributeSelector) {
        rcs.selectorLibrary.setAttributeSelector(data.match(replace.regex.attributeSelectors));
    }
    ...
    if (options.replaceKeyframes) {
        ...
        data = data.replace(replace.regex.keyframes, replaceKeyframes);
        data = data.replace(replace.regex.animationTrigger, replaceAnimations);
    }

    data = data.replace(options.regex, getCssSelector);

    if (!options.ignoreAttributeSelector) {
        data = data.replace(replace.regex.attributeSelectors, getAttributeSelector);
    }
    ...
    function replaceAnimations(match) {
        ...
        const result = splittedGroup.map(group => group.replace(replace.regex.matchFirstWord, replaceKeyframes));

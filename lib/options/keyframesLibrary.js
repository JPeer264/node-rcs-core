...
class KeyframesLibrary {
  ...
  /**
   * fills keyframes with the necessary data based on the given string
   *
   * @param {String} css file
   *
   * @memberof KeyframesLibrary
   */
  fillLibrary(data) {
    // set the keyframes here
    const keyframes = data.match(rcs.replace.regex.keyframes);

    if (Object.prototype.toString.call(keyframes) === '[object Array]') {
      rcs.keyframesLibrary.set(keyframes.map(key => key.split(/\s+/)[1]));
    }
  }
   ...
   *    this could be either a css selector or an array of css selectors
   */
  set(keyframe, renamedKeyframe, options) {
    ...
    if (Object.prototype.toString.call(keyframe) === '[object Array]') {
      keyframe.forEach(item => this.set(item, renamedKeyframe, options));
 ...
 */
module.exports = new KeyframesLibrary();

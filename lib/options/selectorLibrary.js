...
class SelectorLibrary {
    ...
    set(value, renamedSelector, options) {
        ...
        // @todo set exclude for setted attribute selectors
        ...
        if (typeof value === 'object') {
            for (let i = 0; i < value.length; i++) {
                ...
                if (!options.ignoreAttributeSelector) {
                    if (this.isInAttributeSelector(value[i]) || options.preventRandomName) {
                        options.preventRandomName = true;
                    } else {
                        options.preventRandomName = false;
                    }
                }
        ...
        if (!options.ignoreAttributeSelector) {
            if (this.isInAttributeSelector(value) || options.preventRandomName) {
                options.preventRandomName = true;
            } else {
                options.preventRandomName = false;
            }
     ...
     *
     * @param {String} string the attribute selector; e.g.: 'class$="selector"'
     */
    setAttributeSelector(string) {
        ...
        if (typeof string === 'object') {
            for (let value of string) {
                ...
                selector = typeChar + exec[2] + exec[3].slice(1, exec[3].length - 1);

                this.attributeSelectors[selector] = {
                    type: exec[1],
                    typeChar,
    ...
    isInAttributeSelector(string) {
        for (let attributeSelector in this.attributeSelectors) {
            ...
            if (attributeSelector.charAt(0) !== string.charAt(0)) {
                continue;
            }

            if (attributeSelector.charAt(1) === '*') {
                if (string.match(attributeString)) {
                    return true;
                }
            }

      ...
      if (node.type === 'TemplateElement' && 'raw' in node.value) {
        ...
        const raw = node.value.raw.replace(allRegex.templateSelectors, function (txt, p1, p2, p3) {
          ...
          const replacedAttr = p1 === 'class' ?
              newValue.replace(newValue, match => replaceString(match, selectorsLibrary.getClassSelector().getAll({ regex: true }), ' ', { countStats: false }))
            : newValue.replace(newValue, match => replaceString(match, selectorsLibrary.getIdSelector().getAll({ regex: true }), ' ', { countStats: false }));
          // eslint-disable-next-line
